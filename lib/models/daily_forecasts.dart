...
class DailyForecasts {
  ...
  DailyForecasts(
      List<WeatherForecast> forecasts, List<WeatherForecast> hourlyForecasts) {
        ...
        for (final forecast in forecasts) {
          ...
          if (forecastDays.containsKey(increasingKey)) {
            ...
            List<FlSpot> hourlyPrecipForecast = generateHourlyData(
                getYVariable: (forecast) =>
                    forecast.probabilityOfPrecipitation.toDouble(),
                day: day,
                hourlyForecasts: hourlyForecasts);
  ...
  List<FlSpot> generateHourlyData(
      {required Function(WeatherForecast) getYVariable,
      required int day,
      required List<WeatherForecast> hourlyForecasts}) {
    List<FlSpot> tempData = [];
    for (final forecast in hourlyForecasts) {
      if (forecast.startTime.day == day) {
        tempData.add(
            FlSpot(forecast.startTime.hour.toDouble(), getYVariable(forecast)));
      }
    }
    return tempData;
  }
